...
pt_excise <- read.csv("output/discharge_results/miller_creek_discharge_excise.csv") %>%
  transform(start_date = yday(anydate(start_date)),
            end_date = anydate(end_date)) %>%
  select(-notes,-period)

# use anti_join approach as outlined in thermal imagery example
flagged_data <- inner_join(pt,pt_excise)




############## 9/30/22 working here to adapt time series flag function. ############
...
# apply excise table to pt time series
z <- anti

# remove erroneous data with excise table
pt_remove <- pt %>%
