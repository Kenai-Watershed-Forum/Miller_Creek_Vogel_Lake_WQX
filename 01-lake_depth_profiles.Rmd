...
```{r ,echo = FALSE, fig.cap = "June 2022", fig.width=10, fig.height=11}

# filter to jun 2022 data 
jun22_lake_wqx_dat <- lake_wqx_dat %>%
  mutate(month = month(sample_date),
         year = year(sample_date)) %>%
  filter(month == "6",
         year == "2022")
...
jun22_lake_wqx_dat %>%
  ggplot() +
    ...
    theme(panel.spacing = unit(1.2, "lines"),
          strip.text.y = element_text(angle = 360)) +
    ggtitle("June 2022 Water Quality All Sites")
```
