...
## assign "ND" (not detected) as zero
  mutate_all(funs(str_replace(., "ND", "0"))) %>%
...
z <- rot_dat %>%
  group_by(sample_name,field_sample_date) %>%
  summarise(rotenone_avg_ppb = mean(rotenone_ppb))
